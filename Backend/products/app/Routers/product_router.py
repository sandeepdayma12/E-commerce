...
from app.utils.jwt import get_current_admin_id
...
async def create_product(
    ...
    result = svc.create(payload, images, admin_id)
...
@product_router.get("/api/products/by_admin/")
def get_products_by_admin(
    db: Session = Depends(get_db),
    admin_id: int = Depends(get_current_admin_id)
):
    svc = ProductService(db)
    products = svc.get_products_by_admin(admin_id)
    return products
